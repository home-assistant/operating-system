...
function create_disk_image_genimage() {
...
function _create_disk_gpt() {
    ...
    local kernel_img="$(path_kernel_img)"
    local hdd_img="$(hassos_image_name img.old)"
    local hdd_count=${DISK_SIZE:-2G}
    ...
    # Write new image & GPT
    rm -f "${hdd_img}"
    truncate --size="${hdd_count}" "${hdd_img}"
    sgdisk -o "${hdd_img}"
